...
async function createOcrRuntime(): Promise<OcrRuntime> {
  ...
  // Tesseract's worker has no .catch() on its WASM init promise.
  // If WASM compilation fails (e.g. CSP blocks it), createWorker hangs forever.
  // Use a timeout to fail fast with a clear message.
...
async function extractWithOcr(doc: pdfjs.PDFDocumentProxy): Promise<{ pageTexts: string[]; spans: PdfTextSpan[]; extractedChars: number; pagesScanned: number; averageConfidence: number; discardedWords: number; }> {
  ...
  const OCR_SCALE = 1.8;
  const RELAXED_OCR_WORD_CONFIDENCE = 20;
    ...
    for (let pageNumber = 1; pageNumber <= pageCount; pageNumber += 1) {
      ...
      for (const word of words) {
        ...
        if (!hasConfidence || confidence >= MIN_PDF_OCR_WORD_CONFIDENCE) {
          acceptedWords.push({ tokenText, bbox: word.bbox });
          continue;
        }

        if (confidence >= RELAXED_OCR_WORD_CONFIDENCE) {
          relaxedCandidates.push({ tokenText, bbox: word.bbox });
        }
      }

      const wordsToUse = acceptedWords.length > 0 ? acceptedWords : relaxedCandidates;
      if (acceptedWords.length === 0 && relaxedCandidates.length > 0) {
        console.warn(`PDF OCR relaxed confidence fallback applied on page ${pageNumber}.`);
      }

      const keptWordCount = wordsToUse.length;
      const nonEmptyWordCount = acceptedWords.length + relaxedCandidates.length;
      if (nonEmptyWordCount > keptWordCount) {
        discardedWords += nonEmptyWordCount - keptWordCount;
      }
      ...
      for (const word of wordsToUse) {
        ...
        cursor = pushSpan(spans, pageNumber, word.tokenText, cursor, {
          x: x0 / OCR_SCALE,
        ...
        pageTokens.push(word.tokenText);
      }
