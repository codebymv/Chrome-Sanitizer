...
// src/shared/file/decoders/docx.ts
var import_mammoth = __toESM(require_lib3(), 1);
async function decodeDocxFile(file, extension) {
  const buffer = await file.arrayBuffer();
  const result2 = await import_mammoth.default.extractRawText({ arrayBuffer: buffer });
  const extractedText = result2.value.trim();
  const warningText = result2.messages.length > 0 ? `
